...
from src.common import kosu as KOSU              # NURON: kosular/folder<N> (2026-09-17)
...
class ConnectionHandler:
    def __init__(self, base_url, username=None, password=None):
        ...
        self.classes = None
        self.video_name = ''
        self.img_save_path = KOSU.alt("_images") + "/"      # NURON: kosular/folder<N>/_images/
    ...
    # ══════════════════════════════════════════════════════════════════════════
    #  NURON — YEREL AYNA (2026-09-13).  Sunucuya giden tahmin paketinin BIREBIR
    #  kopyasi result/<oturum>/predictions_<zaman>.jsonl dosyasina eklenir.
    #  SUNUCUYA GIDEN VERI HIC DEGISMEZ: burada yalnizca hazir `payload` METNI
    #  okunur, hicbir sey yazilmaz/degistirilmez ve tum govde try/except icindedir
    #  (kayit hatasi gonderimi ETKILEMEZ). Her kosu AYRI dosyaya yazar; eski
    #  dosyanin uzerine yazilmaz. Bicim server/e2e_analiz.py ile uyumludur:
    #  {"frame_index", "source_frame", "frame", "t", "payload"} — yarisma sonrasi
    #  ayni analiz araclariyla incelenebilir.
    # ══════════════════════════════════════════════════════════════════════════
    def _sonuc_yaz(self, payload):
        try:
            yol = getattr(self, "_sonuc_yol", None)
            if yol is None:
                oturum = "".join(c if (c.isalnum() or c in "-_.") else "_"
                                 for c in str(self.video_name or "oturum").strip("/")) or "oturum"
                klasor = os.path.join(KOSU.alt("result"), oturum)   # NURON: kosular/folder<N>/result/
                os.makedirs(klasor, exist_ok=True)
                yol = os.path.join(klasor, "predictions_%s.jsonl" % time.strftime("%Y%m%d_%H%M%S"))
                self._sonuc_yol = yol
                with open(os.path.join(klasor, "oturum.json"), "w", encoding="utf-8") as f:
                    json.dump({"oturum": oturum, "base_url": self.base_url,
                               "baslangic": time.strftime("%Y-%m-%d %H:%M:%S"),
                               "dosya": os.path.basename(yol)}, f, ensure_ascii=False, indent=1)
                logging.info("[NURON] Yerel sonuc aynasi: %s", yol)
            p = json.loads(payload)
            im = p.get("frame") or ""
            sayilar = "".join(c if c.isdigit() else " " for c in str(im)).split()
            kayit = {"frame_index": int(sayilar[-1]) if sayilar else None,
                     "source_frame": im, "frame": im, "t": time.time(), "payload": p}
            with open(yol, "a", encoding="utf-8") as f:
                f.write(json.dumps(kayit, ensure_ascii=False) + "\n")
        except Exception as e:          # kayit ASLA gonderimi bozmaz
            if not getattr(self, "_sonuc_uyari", False):
                self._sonuc_uyari = True
                logging.warning("[NURON] Yerel sonuc aynasi yazilamiyor (gonderim etkilenmedi): %s", e)
    ...
    def send_prediction(self, prediction, retries=5, initial_wait_time=0.1):
        ...
        self._sonuc_yaz(payload)        # NURON: yerel ayna (yukariya bak) — payload'a DOKUNMAZ
        files = []
        headers = {

...
def temas(ms):
    """Komsuluk grafigi. Ic-ice olanlar komsu SAYILMAZ; her dugumde yalnizca
    EN YAKIN KOM_UST komsu tutulur.

    YAKINLIK = komsunun EN UZAK pikselinin adaya uzakligi / adanin kutu kosegeni
    (yonlu Hausdorff). Boylece "bir ucuyla degip otekine uzayan" serit komsu
    SAYILMAZ; komsunun TAMAMI adanin yaninda olmali.
    """
    n = len(ms)
    ...
    # her ada icin uzaklik alani: o adaya olan piksel uzakligi
    uzak = [cv2.distanceTransform((~m).astype(np.uint8), cv2.DIST_L2, 3) for m in ms]
    kos = [max(1.0, float(np.hypot(k[2]-k[0], k[3]-k[1]))) for k in ku]
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            if xj2 < xi1-6 or xj1 > xi2+6 or yj2 < yi1-6 or yj1 > yi2+6:
                continue
            if not (gen[i] & ms[j]).any():                  # degmiyorsa komsu degil
                continue
            if float((ms[i] & ms[j]).sum()) / min(al[i], al[j]) >= A.IC_ESIK:
                continue                                     # ic ice
            d = float(uzak[i][ms[j]].max()) / kos[i]          # komsunun EN UZAK pikseli
            if d > A.KOM_UZAK:                                # bambaska yere uzuyor -> ELE
                continue
            out[i].append((j, d))
    for i in range(n):

...
_ONBELLEK = {"anah": None, "ms": None}   # ayni kare + ayni cozunurluk icin FastSAM bir kez (ikili motor)


def bol(img):
    """kare -> maske listesi (ayrik, minimum alan suzgecinden gecmis)"""
    anah = (id(img), img.shape, A.MAXSIDE, A.FASTSAM_CONF)
    if _ONBELLEK["anah"] == anah:
        return [m.copy() for m in _ONBELLEK["ms"]]
    ms = _bol(img)
    _ONBELLEK["anah"], _ONBELLEK["ms"] = anah, [m.copy() for m in ms]
    return ms
